...
1. 前往 [Releases](https://github.com/pdajoy/mindshelf/releases) 下载最新的 Chrome 扩展 zip
...
从 `.env.example` 创建 `backend/.env`，或通过 Docker 环境变量传入：
...
# Obsidian 导出（文件直写，无需 Obsidian 运行）
OBSIDIAN_VAULT_PATH=/path/to/vault
```

完整配置项参见 [`.env.example`](backend/.env.example)。
